...
$sql = "select * from membership where id like '%$search%' order by id asc";
$result = mysqli_query($con, $sql) or die("실패원인12 " . mysqli_error($con));

while ($row = mysqli_fetch_array($result)) {
    $sid = $row['id'];
 
    echo "<input type='button' value='$sid' class='abcd' style='padding : 5px 5px; border:0px; background-color:white;'><br>";
}
